...
DARK_NAVY = HexColor("#152240")
GOLD = HexColor("#C9A84C")            # Brand gold (UI accent)
CREAM = HexColor("#F5F0E8")
WARM_WHITE = HexColor("#FDFBF7")
LIGHT_GRAY = HexColor("#E0DCD4")
MID_GRAY = HexColor("#6B6560")        # Warm Gray (brand neutral)
DARK_GRAY = HexColor("#4A4A4A")
FIELD_DOT = HexColor("#C8C3BA")
RED_ACCENT = HexColor("#8B2635")      # Wax Seal Red (formal/print accent)
ACTION_BG = HexColor("#EEF2F7")       # Soft blue-gray for action guide blocks
ACTION_BORDER = HexColor("#1B2D4F")   # Deep Navy border
...
class ResolvedBriefBuilder:
    ...
    def build_cover(self, c):
        ...
        for i in range(max_rows):
            y = y_start - (i * 24)
            if i < len(sections_left):
                c.setFillColor(GOLD)
                c.circle(80, y + 4, 4, fill=1, stroke=0)
                c.setFillColor(NAVY)
                c.drawString(92, y, sections_left[i])
            if i < len(sections_right):
                c.setFillColor(GOLD)
                c.circle(W/2 + 20, y + 4, 4, fill=1, stroke=0)
                c.setFillColor(NAVY)
                c.drawString(W/2 + 32, y, sections_right[i])
